...
| **场景拓展与增强** | 5/11 ~ 5/24（第十一至十二周） | 完成**页面切换、拍照、浏览器**三个场景的基础部分（eBPF 采集 + Perfetto 配置 + 分析工具 + 多 App 测试）；推进**支付、游戏、视频**三个拓展场景的基础部分；并行进行页面切换、拍照、浏览器的增强部分（深度帧分析、系统开销归因、评分模型优化） | 基础部分：潘智勇，邵晨轩，杨子皓 增强部分：李松茂，贺小轩 |
